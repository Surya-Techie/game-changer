...
# ─── Measured calibration (walk-forward event study on real NSE data) ──────
# calibration.json is generated by replaying every detector bar-by-bar over
# real history and simulating each detection's own target/stop race. Where a
# pattern has enough samples, its MEASURED stats replace the literature
# guesses above — the literature numbers describe other markets and other
# exit rules, not this engine.

_CALIBRATION: dict[str, dict] = {}
_CALIBRATION_MIN_SAMPLES = 15
try:
    import json as _json
    import os as _os

    _cal_path = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "calibration.json")
    if _os.path.exists(_cal_path):
        with open(_cal_path) as _f:
            _raw = _json.load(_f)
        _CALIBRATION = {k: v for k, v in _raw.items() if not k.startswith("_") and isinstance(v, dict)}
except Exception:  # noqa: BLE001 — calibration is an enhancement, never fatal
    _CALIBRATION = {}


def measured_stats(name: str) -> Optional[dict]:
    """Measured {win_rate, expectancy_r, samples, ...} for a pattern, or None
    when the calibration study lacks enough samples to be trustworthy."""
    c = _CALIBRATION.get(name)
    if c and int(c.get("samples", 0)) >= _CALIBRATION_MIN_SAMPLES:
        return c
    return None


def baseline_win_rate(name: str) -> float:
    """Measured (calibrated) win rate when available; else the conservative
    literature baseline; unknown name → 0.50 (coin flip)."""
    c = measured_stats(name)
    if c is not None:
        return float(c["win_rate"])
...
def ensure_df(data: Any) -> pd.DataFrame:
    ...
    A frame this function already normalised is returned AS-IS (no copy).
    Detector sweeps call ensure_df dozens of times per bar on the same
    frame; skipping the copy + per-column coercion on re-entry is what
    keeps historical scans (power analysis) inside the API timeout.
    Callers must not mutate the returned frame in place.
    """
    if isinstance(data, pd.DataFrame) and data.attrs.get("_qti_ensured"):
        return data
